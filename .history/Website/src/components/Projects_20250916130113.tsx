...
export const Resume = () => {
  ...
  const [currentCharacterImage, setCurrentCharacterImage] = useState<string>('assets/indigoleague1.png');
  ...
  const toggleSkillSection = (section: string) => {
    ...
    // Update character image based on section
    const sectionImageMap: { [key: string]: string } = {
      'languages': 'assets/LanguagePic.jpg',
      'health': 'assets/FitnessPic.jpg',
      'technical': 'assets/TechnicalPic.png',
      'hobbies': 'assets/HobbyPic.JPG',
      'attributes': 'assets/AttributesPic.png',
      'teams': 'assets/MyteamPIc.png'
    };
    
    if (section && sectionImageMap[section]) {
      setCurrentCharacterImage(sectionImageMap[section]);
    } else {
      // Default image when closing a section
      setCurrentCharacterImage('assets/indigoleague1.png');
    }
  };
  ...
  return (
    <section id="resume" className="bg-gradient-to-b from-[#3A5466] via-[#2F4553] via-[#243441] to-[#1A252F]">
      {/* Timeline Section */}
      <div className="py-20">
                  ...
                  <div className={`flex ${item.side === 'left' ? 'justify-start pr-1/2' : 'justify-end pl-1/2'}`}>
                    <div className={`w-5/12 ${item.side === 'right' ? 'ml-auto' : 'mr-auto'}`}>
                      <div className="group relative">
                        {/* Compact title card (shown when not hovered) */}
                        <div className={`p-4 rounded-lg cursor-pointer transition-all duration-300 transform group-hover:opacity-0 group-hover:scale-105 flex flex-col justify-center min-h-[120px] ${
                          item.side === 'left'
                            ? 'bg-[#243447] text-right'
                            : 'bg-[#1E2A3B] text-left'
                        }`}>
                          {/* Type badge */}
                          <div className={`inline-block px-3 py-1 rounded-full text-xs font-medium text-white mb-2 ${getTypeColor(item.type)} ${item.side === 'left' ? 'ml-auto' : 'mr-auto'}`}>
                            {getTypeLabel(item.type)}
                          </div>

                          <h3 className="text-lg font-semibold text-[#F0F0E8] mb-1">
                            {item.title}
                          </h3>
                          {item.company && (
                            <p className="text-[#7A9CA9] font-medium text-sm">{item.company}</p>
                          )}
                          {item.location && (
                            <p className="text-[#F0F0E8]/70 text-xs">{item.location}</p>
                          )}
                        </div>

                        {/* Expanded details card (shown on hover) */}
                        <div className={`absolute top-0 left-0 right-0 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 pointer-events-none group-hover:pointer-events-auto transform group-hover:scale-105`}>
                          <div className={`p-6 rounded-lg bg-[#1E2A3B] shadow-2xl border border-[#7A9CA9]/30 ${item.side === 'left' ? 'text-right' : 'text-left'}`}>
                            ...
                            <p className="text-[#F0F0E8]/80 text-sm leading-relaxed">
                              {item.description}
                            </p>
                            {item.link && (
      ...
      <div className="py-20 px-4" id="project-portfolio">
                  ...
                  <div className="relative">
                    ...
                    <div className={`bg-[#F0F0E8] rounded-lg shadow-2xl border-4 border-[#2C3E50] transition-all duration-500 ease-out ${
                      ...
                      <div className={`transition-all duration-500 ease-out overflow-hidden ${
                        ...
                        {openFolder === 'tandem-app' ? (
                          <div className="space-y-4">
                            <h4 className="text-2xl font-bold text-[#2C3E50] mb-3">Couples Life Tracker App</h4>
                            <p className="text-[#2C3E50] leading-relaxed">
                              A shared lifestyle management tool for two users that centralizes daily tracking into a modular, Duolingo-inspired interface. Built with React Native (Expo Router), PostgreSQL + Prisma, and Context API for global state.
                            </p>
                            <div className="text-[#2C3E50] text-sm space-y-2">
                              <p><strong>Key features:</strong> Profile system, dual-user expense tracking, meals & restaurants logging, grocery lists with store maps, watchlist for movies/shows, reminders with emoji tags, and calendar for anniversaries.</p>
                              <p><strong>In progress:</strong> AI recommendation system using logged habits and ratings to generate personalized meal, activity, and media suggestions with vector database integration for semantic search.</p>
                            </div>
                          </div>
                        ) : (
                          <div
                          ...
                          >
                            Couples Life Tracker App - Shared Lifestyle Management
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col items-center">
              <div className="bg-[#F0F0E8] rounded-xl p-16 shadow-xl border-2 border-[#7A9CA9]/30 w-full max-w-md">
                <h4 className="text-2xl font-bold text-[#2C3E50] text-center mb-8">Character Profile</h4>
                ...
                <div className="flex justify-center mb-8">
                  <img
                    ...
                    className="w-48 h-48 object-cover rounded-lg shadow-lg border-2 border-[#7A9CA9]/30 transition-all duration-500 ease-in-out"
                  />
                </div>
                ...
                <div className="space-y-4 text-center">
                  <div className="flex justify-between items-center">
                    <span className="text-[#2C3E50] font-medium text-lg">Level:</span>
                    <span className="text-[#7A9CA9] font-bold text-lg">23</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[#2C3E50] font-medium text-lg">Class:</span>
                    <span className="text-[#7A9CA9] font-bold text-lg">Data Analyst</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[#2C3E50] font-medium text-lg">Location:</span>
                    <span className="text-[#7A9CA9] font-bold text-lg">Atlanta, GA</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                {expandedSkillSection === 'health' && (
                  <div className="bg-[#1E2A3B] p-4 space-y-4">
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        <span>5K Speed</span>
                        <span className="text-emerald-400">18% faster than avg person</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        <span>Endurance</span>
                        <span className="text-emerald-400">14% faster than avg person</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        ...
                        <span className="text-emerald-400">26% higher than avg person</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        ...
                        <span className="text-emerald-400">66% stronger than avg person</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        ...
                        <span className="text-emerald-400">3x stronger than avg person</span>
                      </div>
                      <div className="flex items-center space-x-4">
              ...
              <div className="border border-[#7A9CA9]/30 rounded-lg overflow-hidden">
                ...
                {expandedSkillSection === 'hobbies' && (
                  <div className="bg-[#1E2A3B] p-4 space-y-4">
                    <div className="text-xs text-[#7A9CA9] mb-4 italic">
                      Personal passion & time investment levels
                    </div>
                    
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        <span>Pokémon</span>
                        <span className="text-purple-400">Lifelong obsession</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        ...
                        <span className="text-purple-400">Weekly playing & watching</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        <span>Reading</span>
                        <span className="text-purple-400">Regular reader, philosophy focus</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        <span>Cooking</span>
                        <span className="text-purple-400">Learning with girlfriend</span>
                      </div>
                      <div className="flex items-center space-x-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs text-[#F0F0E8]">
                        ...
                        <span className="text-purple-400">Hands-on learning hobby</span>
                      </div>
                      <div className="flex items-center space-x-4">
              ...
              <div className="border border-[#7A9CA9]/30 rounded-lg overflow-hidden">
                ...
                {expandedSkillSection === 'attributes' && (
                  <div className="bg-[#1E2A3B] p-4 space-y-4">
                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Coachability</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-yellow-500 to-yellow-400 h-4 rounded-full transition-all duration-1000" style={{ width: '85%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">85</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-yellow-500 to-yellow-400 h-4 rounded-full transition-all duration-1000" style={{ width: '90%' }}></div>
              ...
              <div className="border border-[#7A9CA9]/30 rounded-lg overflow-hidden">
                ...
                {expandedSkillSection === 'teams' && (
                  <div className="bg-[#1E2A3B] p-4 space-y-4">
                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Borussia Dortmund</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-orange-500 to-orange-400 h-4 rounded-full transition-all duration-1000" style={{ width: '100%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">100</span>
                      </div>
                    </div>

                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Georgia Bulldogs</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-orange-500 to-orange-400 h-4 rounded-full transition-all duration-1000" style={{ width: '100%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">100</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Pumas UNAM</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-orange-500 to-orange-400 h-4 rounded-full transition-all duration-1000" style={{ width: '100%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">100</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Red Bull F1</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-orange-500 to-orange-400 h-4 rounded-full transition-all duration-1000" style={{ width: '100%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">100</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-20 text-sm text-[#F0F0E8] font-medium">Barcelona FC (Womens)</div>
                      <div className="flex-1 bg-[#1E2A3B] rounded-full h-4 relative">
                        <div className="bg-gradient-to-r from-orange-500 to-orange-400 h-4 rounded-full transition-all duration-1000" style={{ width: '100%' }}></div>
                        <span className="absolute right-2 top-0 text-xs text-[#F0F0E8] leading-4">100</span>
                      </div>
